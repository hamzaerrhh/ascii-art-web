...
export default function Alert(theme) {
  ...
  const getPaletteColor = (severity) =>
    severity ? vars.palette[severity] : vars.palette.info;

  const getCommonStyles = (ownerState) => {
    const isWarningOrSuccess =
      ownerState.severity === "warning" || ownerState.severity === "success";
  ...
  const filledVariant = ({ ownerState }) => {
    ...
    return {
      color: isWarningOrSuccess
        ? vars.palette.common.black
        : vars.palette.common.white,
      backgroundColor: isWarningOrSuccess
        ? paletteColor.dark
        : paletteColor.main,
      "& .MuiAlert-icon": {
        color: isWarningOrSuccess
